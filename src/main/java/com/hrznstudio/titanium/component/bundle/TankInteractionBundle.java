    ...
    public TankInteractionBundle(Supplier<LazyOptional<IFluidHandler>> fluidHandler, int posX, int posY, T componentHarness, int maxProgress) {
        ...
        this.bar = new ProgressBarComponent<T>(posX + 5, posY + 30, maxProgress)
            ...
            .setCanIncrease(t -> {
                ...
                return !this.input.getStackInSlot(0).isEmpty() && this.input.getStackInSlot(0).getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY).isPresent() && !stack.isEmpty() && !ItemStack.isSameItemSameTags(stack, this.input.getStackInSlot(0)) && (this.output.getStackInSlot(0).isEmpty() || ItemHandlerHelper.canItemStacksStack(stack, this.output.getStackInSlot(0)));
            })
            .setOnFinishWork(() -> {
